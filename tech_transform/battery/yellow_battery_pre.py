...
def transform_battery_pre(df, file_name, table_name):
    try:
        ...
        log_csv_path = os.path.join(project_root, 'loaded_files', 'yellow_battery_pre_files.csv')
...
def log_processed_filename(file_name, log_csv_path):
    ...
    try:
        ...
        base_file_name = base_file_name.replace('_Pre-Estimation', '').replace('_Preestimation', '')
