...
def transform_solar_assump(df, df_pre, file_name, table_name):
    try:
        ...
        df = solar_SA11(df, 'S_A_1_1')
        df = df.dropna(axis=1, how='all')
        df = solar_estimator_project_manager_transformation(df)
        df = solar_assumption_transformation(df, file_name)

        # If pre-estimation data is available, correlate it with assumption data
        if df_pre is not None:
            pre_uid_letter_dict = solar_correlate_preestimation_uid_with_letter(df_pre)
            assump_uid_letter_dict = solar_correlate_assumption_uid_with_letter(df)
...
def solar_assumption_transformation(df, file_name):
    """
...
# Solar Estimator Project Manager Transformation 
def solar_estimator_project_manager_transformation(df):
    """
...
# Helper Functions
def solar_SA11(df, new_row_label):
    """
    Processes the DataFrame to:
    1. Add a new dynamic row with a specified label.
    2. Replace 'S_A_1' with 'S_A_1_1' in the DataFrame.
    ...
    for column in df.columns:
        if df[column].eq("S_A_1").any():
            index = df[df[column] == "S_A_1"].index[0]
            df.loc[index, column] = "S_A_1_1"
...
def solar_correlate_preestimation_uid_with_letter(df):
    """
    Create a dictionary linking each Preestimation_UID to its corresponding letter
    based on rows where Variable_UID is 'S_ER_15'.
    ...
    """
    # Filter rows where Variable_UID is 'S_ER_15'
    filtered_df = df[df['Variable_UID'] == 'S_ER_15']
...
def solar_correlate_assumption_uid_with_letter(df):
    """
    Create a dictionary linking each Assumption_UID to its corresponding letter
    based on rows where Variable_UID is 'S_A_1_1'.
    ...
    """
    # Filter rows where Variable_UID is 'S_A_1_1'
    filtered_df = df[df['Variable_UID'] == 'S_A_1_1']
